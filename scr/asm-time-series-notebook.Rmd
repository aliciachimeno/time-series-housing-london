...
forecast_df <- data.frame(
  ...
  Lower95 = c(rep(NA, length(historical_data)), exp(as.numeric(forecast_results$lower[, 2]))),
  Upper95 = c(rep(NA, length(historical_data)), exp(as.numeric(forecast_results$upper[, 2])))
)
...
ggplot(forecast_df, aes(x = Time, y = Value, color = Type)) +
  ...
  geom_ribbon(data = subset(forecast_df, Type == "Forecast"),
              aes(ymin = Lower95, ymax = Upper95), fill = "lightblue", alpha = 0.4) +  # 95% CI
  labs(title = "Time Series with 1-Year Forecast",
       x = "Time",
