...
we observe a significant last lag after which subsequent lags are nearly zero. This suggests that an AR(p) model may be appropriate. 
...
With the plot we observed some characteristics of the time series, but we will check wich is the best model numerically by trying to optimize the metrics: AIC (minimize), loglike () and BIC. 

Manually we can see that 
```{r}
arima(d1lnserie, order = c(0, 1, 1)) 
arima(d1lnserie, order = c(0,1,1))
arima(d1lnserie, order = c(1,0,1))
arima(d1lnserie, order = c(1,1,1))
```
the best one is p=q=1 and d=0. 

We will create a function that selects the best arima model varying the p ,d and q values. 
...
find_best_arima <- function(time_series, max_p = 8, max_q = 8, max_d = 1) {
  ...
  best_model <- NULL
  results <- foreach(p = 0:max_p, .combine = rbind, .packages = 'forecast') %:%
            foreach(d = 0:max_d, .combine = rbind) %:%
            foreach(q = 0:max_q, .combine = rbind) %dopar% {
              try({
                model <- arima(time_series, order = c(p, d, q))
                c(aic = model$aic, p = p, d = d, q = q)
              }, silent = TRUE)
            }
  stopCluster(cl)
  best_result <- results[which.min(results[, 1]), ]
  best_aic <- best_result[1]
  best_order <- best_result[2:4]
  return(list(best_order = best_order, best_aic = best_aic))
}

# Use the function to find the best model
result <- find_best_arima(d1lnserie)

# View the best model parameters
result$best_order
result$best_aic

best_model <- arima(d1lnserie, order = c(5,0,8))
best_model
```
