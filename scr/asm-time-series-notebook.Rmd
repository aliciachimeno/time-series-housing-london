...
- **Normality tests**: Shapiro-Wilk Test, Anderson-Darling Test, Jarque-Bera Test. All tests have p-values > 0.05, indicating that the residuals are approximately normally distributed.

- **Residual Plots**: The residuals do not seem to be autocorrelated, they seem to behave like white noise, therefore the model captures the structure of the data adequately.

- **QQ-plot**: Data seems to be normal distributed because almost all the data follow the line.  

- **Breusch-Pagan test** checks for heteroskedasticity (non-constant variance of residuals). The p-value (1) is much greater than 0.05, so we fail to reject the null hypothesis. This indicates no significant evidence of heteroskedasticity in the residuals. Residual variance is constant.

- **Durbin-Watson Test** checks for autocorrelation in residuals. The DW statistic of ~2 is close to the ideal value for no autocorrelation, and the p-value (1) is much greater than 0.05. This suggests no evidence of significant autocorrelation in the residuals.

**Ljung-Box Test** At all tested lags (1, 2, and 3), the Ljung-Box test shows very small p-values (much less than 0.05). This indicates strong evidence of autocorrelation in the residuals. 
...
##### Model 1. AR(1)
**Causality and inveritbility**: The model is causal. Causal because the inverse roots of AR lay inside the circle. It is not invertible because does not have an MA component. 


##### Model 2. ARIMA(1,1)
**Causality and inveritbility**: The model is both causal and invertible. Causal because the inverse roots of AR lay inside the circle. Invertible because the inverse roots of MA lay inside the circle.
**Adequacy measures**:
...
##arima(train). necessitem fer arima train and arima test ------ 
forecast_values <- forecast(model1, h = 12)
plot(forecast_values, main = "ARIMA Forecast vs Actual", col = "blue")
...
We will use all the data on the model to predict the next following 12 months. 
